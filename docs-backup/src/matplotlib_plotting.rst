    ...
    plot_configs:
      ...
      artists:
        dt-station-global: &dt-station-global 
          src: "ydana.utils.dt_plot_functions.embed_dts2axes"
          rep-info:
            particle_type: 'digis'
            cmap_var: 'time'
          bounds_kwargs:
        ...
        dt-station-local:
          <<: *dt-station-global
          src: "ydana.utils.dt_plot_functions.embed_dt2axes"

        dt-am-tps-global:
          src: "ydana.utils.dt_plot_functions.embed_segs2axes_glob"
          rep-info:
            particle_type: 'tps'
            cmap_var: 'quality'
          segs_kwargs:
...
.. note::
    Each artist is a Python function (see ``ydana/utils/dt_plot_functions.py``) that receives the event 
...
ydana combines your configuration, the event data, and Matplotlib to create these visualizations.

The plotting process involves the ``ydana`` CLI dispatching to the ``plot_dt_chamber`` function, which then 
...
.. mermaid::
    ...
    sequenceDiagram
        ...
        User->>ydana CLI: ydana plot-dt ...
        ydana CLI->>RUN_CONFIG: Load/Update config (plot_configs, artists)
        ydana CLI->>plot_dt_chamber: Call plot_dt_chamber(inpath, ...)
        ...
        plot_dt_chamber-->>ydana CLI: Done
        ydana CLI-->>User: Output messages / Image file

**Explanation of the Flow:**

1.  **CLI Invokes ``plot_dt_chamber``**: You run the ``ydana plot-dt`` command. The CLI (after configuring ``RUN_CONFIG``) 
calls the ``plot_dt_chamber`` function from ``ydana/analysis/plot_dt_chamber.py``.
...
3.  **Parse Plot Configuration**: It then calls ``parse_plot_configs()`` (from ``ydana/utils/functions.py``) which consults ``RUN_CONFIG`` to get the ``mplhep-style``, ``figure-configs``, and a dictionary of all available ``artist_builders`` (Python functions linked to artist names).
...
5.  **Build and Embed Artists**: For each ``artist-name`` you specified in the CLI (e.g., ``dt-station-local``), ``plot_dt_chamber`` retrieves the corresponding ``artist_builder`` function from its dictionary. It calls this ``artist_builder`` function, passing it the ``Event`` object, the chamber location (``wheel``, ``sector``, ``station``), and the Matplotlib ``Axes`` objects.
    ...
    *   The ``artist_builder`` function (e.g., ``embed_dt2axes`` in ``ydana/utils/dt_plot_functions.py``) then performs the actual drawing. It filters the ``Event``'s particles (e.g., ``digis``) for the specific chamber, and uses specialized ``mpldts`` classes like ``DTStationPatch`` to create visual elements (patches) that represent the detector geometry and data. These patches are then added to the Matplotlib ``Axes``.
...
.. code-block:: bash
    ...
    ydana plot-dt -i [INPATH] -o [OUTPATH] -cf [CONFIG_FILE] -evn 0 --wheel 2 --sector 1 --station 1 --artist-names [ARTIST_NAMES ...]
...
.. code-block:: bash
    ...
    ydana plot-dts -i [INPATH] -o [OUTPATH] -cf [CONFIG_FILE] -evn 0 --artist-names [ARTIST_NAMES ...]
...
Suppose you want to overlay a scatter plot of simHits on top of your digis. First, define a function, for instance, in ``ydana/utils/dt_plot_functions.py``:

.. literalinclude:: ../../ydana/utils/dt_plot_functions.py
    :language: python
    :lines: 480-545

Then, add it to your configuration:

.. literalinclude:: ../../ydana/utils/yamls/run_config.yaml
    :language: yaml
    :dedent: 2
    :lines: 309-312
...
plot-dt
-------
.. automodule:: ydana.analysis.plot_dt_chamber
    :members: make_dt_plot, plot_dt_chamber

plot-dts
--------

.. automodule:: ydana.analysis.plot_dt_chambers
