...
+--------------+---------------------------------------------------------------------+--------------------------------------------+
| ``branch``   | Rebuild list-valued attributes from companion ``[flat_ids,          | ``['tps_matched_segments_flat_ids',        |
| (pair)       | flat_counts]`` branches generated by the event dumper.              | 'tps_matched_segments_flat_counts']``      |
+--------------+---------------------------------------------------------------------+--------------------------------------------+
| ``expr``     | Calculate using a Python expression.                                | ``'time // 25'``                           |
+--------------+---------------------------------------------------------------------+--------------------------------------------+
| ``src``      | Call a Python function, passing the ``Particle`` itself to a        | ``'path.to.module.get_detector_side'``     |
|              | callable.                                                           |                                            |
+--------------+---------------------------------------------------------------------+--------------------------------------------+
...
Reading Flattened Nested Collections
------------------------------------

When reading dumped files, nested matching collections are represented with two
branches per attribute: one branch with concatenated values (``*_flat_ids``)
and one branch with per-particle counts (``*_flat_counts``).

You can define those attributes as a two-item ``branch`` list:
...
For particle index ``i``, the framework reconstructs the corresponding list using:

.. math::

    \mathrm{start} = \sum_{k=0}^{i-1} \mathrm{counts}[k], \qquad
    \mathrm{end} = \mathrm{start} + \mathrm{counts}[i]

and returns:

.. math::

    \mathrm{ids}[\mathrm{start}:\mathrm{end}]

Validation rules:

- The list under ``branch`` must contain exactly two elements.
- Both companion branches must exist in the event entry.
- The particle index must be within the bounds of ``flat_counts``.
