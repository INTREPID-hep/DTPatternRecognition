"""Analysis dispatch for the ``dtpr fill-histos`` command.

This module acts as the CLI entry-point for histogramming. It loads the 
event NTuples, reads histogram definitions via :func:`dtpr.base.histos.from_config`, 
and delegates all distributed filling logic to :func:`dtpr.base.histos.fill`.
...
def fill_histos(
    ...
    histos_src : str or None
        ...
        ``None`` → read sources from :data:`~dtpr.base.config.RUN_CONFIG`.
    """
    if verbose:
