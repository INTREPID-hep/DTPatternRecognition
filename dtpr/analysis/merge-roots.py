"""Merge per-partition event ROOT files — ``dtpr merge-roots``.
...
::
    ...
    dtpr dump -i /files/ -o results/ -c 8 --per-partition --to-root
    # → results/roots/dumpedEvents_000.root … dumpedEvents_NNN.root

    # Merge when all partitions are done:
    dtpr merge-dumps -i "results/roots/dumpedEvents_*.root" -o results/
"""
...
def merge_roots(
    ...
    try:
        ...
        color_msg(f"Fatal error: hadd failed with exit code {exc.returncode}", "red", indentLevel=1)
        print(exc.stderr)
