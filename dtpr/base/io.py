...
def dump_to_parquet(
    ...
    **Lazy Execution (Dask):**
    ...
    if not _is_dak(events):
        outpath = os.path.join(outfolder, f"dumpedEvents.parquet")
        if not overwrite and os.path.exists(outpath):
            if verbose:
                color_msg(f"{log_label} Skipping existing {outpath} (resume-safe)", "yellow", 1)
            return
        if verbose:
    ...
    if verbose:
        color_msg(f"{log_label} Parquet export × {npartitions} partition(s) | {sched_label}", "blue", 1)
...
def _flatten_awkward_to_dict(array, prefix: str = "", depth: int = 0) -> dict:
...
def dump_to_root(
    ...
    **Branch layout & Fallback:**
    ...
    if verbose:
        color_msg(f"{log_label} ROOT export × {npartitions} partition(s) | {sched_label}", "blue", 1)
    ...
    with ProgressBarFactory(mode="lazy", show=verbose, delay=0.25, desc=desc, ascii=True, unit=" part"):
