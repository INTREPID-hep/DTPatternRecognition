...
from .old_particle import Particle  # Import the base Particle class
    ...
    def _build_particles(self, ev, ptype, pinfo):
        ...
        for i in range(num_particles):
            ...
            if "filter" in pinfo:  # Only keep the particles that pass the filter, if defined
                ...
                if eval(filter_expr, {}, {"p": _particle, "ev": ev, **{k: v for k, v in _particle.__dict__.items() if not k.startswith('_')}}):
        ...
        if "sorter" in pinfo:  # Sort the particles if a sorter is defined
            ...
            _particles = sorted(
                _particles,
                key=lambda _p, _ev=ev: eval(key_expr, {}, {"p": _p, "ev": _ev, **{k: v for k, v in _p.__dict__.items() if not k.startswith('_')}}),
