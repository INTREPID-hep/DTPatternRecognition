...
class basePlotter(object):
     ...
     def add_text(self, text, coords, textSize = 0.07):
            ...
            note.SetTextFont(42)
            note.AddText(text)
            self._noDelete[text] = note; ## so it doesn't get deleted by PyROOT
            return note 

     def doSpam(self, textSize = 0.07):
            align=12 
            text = "#bf{CMS} Simulation"
            cmsprel = self.add_text(text, (self.cmsprel_x1, self.cmsprel_y1, self.cmsprel_x2, self.cmsprel_y2))
