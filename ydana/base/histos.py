...
def fill(
    ...
    **in-place**, and a dictionary of the final objects is returned.
    ...
    if not _is_dak(events):
        if len(events) == 0:
            warnings.warn(
                "Events array is empty (0 rows). Skipping histogram fill.", stacklevel=2
            )
            return {}
        _is_eager = True
    # ────────────────────────────────────────────────────────────────────────
    ...
    if _is_eager:
        desc = color_msg(
            f"{log_label} Filling histograms", "purple", 1, return_str=True
        )
    ...
    # ------------------------------------------------------------------
        ...
        if per_partition:
            ...
            for i, part in enumerate(events.to_delayed()):
                out_path = os.path.join(
                    out_dir, f"histograms{tag}_{str(i).zfill(pad)}.root"
                )
            ...
            desc = color_msg(
                f"{log_label} Processing partitions", "purple", 1, return_str=True
            )
        ...
        # Compute graph
        desc = color_msg(
            f"{log_label} Processing graph nodes", "purple", 1, return_str=True
        )
