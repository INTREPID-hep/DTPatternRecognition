...
def _partition_inputs(
    files: str | FormattedFiles, step_size: int
) -> str | FormattedFiles:
    """
...
def _load_from_root(
    ...
    if schema_section is not None:
        ...
        effective_schema = getattr(
            config, "Schema", None
        )  # global fallback; None → BaseSchema
...
def _print_files_block(
    loaded_files: dict, ds_treename: str = "", max_files: int = 3
) -> None:
    """
...
class NTuple:
    ...
    def __init__(
        ...
        if is_single_input:
            ...
            if not datasets:
                ...
                isinstance(datasets, list) and all(isinstance(d, str) for d in datasets)
            ):
                raise ValueError(
                ...
                load_tasks.append(
                    {
                        ...
                        else ds_cfg.get("treename"),
                        "step": step_size
                        if step_size is not None
                        else ds_cfg.get("step_size"),
                        "split_rg": split_row_groups
    ...
    def _load_events(
        ...
        if pre_steps:
            return execute_pipeline(
                events, pre_steps, dataset=name if name != "inputs" else None
            )
    ...
    def info(self, max_files: int = 3, indent: int = -1) -> None:
        ...
        # ── inputs mode ───────────────────────────────────────────────
            ...
            _print_dataset_block(
                "inputs", loaded, npart, max_files=max_files, indent=indent
            )
